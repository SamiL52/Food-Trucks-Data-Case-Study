...
def save_all_data(all_data: dict[str: pd.DataFrame]) -> None:
    """Saves all data in given dict to separate csv files"""
    for key in all_data:
...
def clean_transaction_data(transaction_df: pd.DataFrame) -> pd.DataFrame:
    ...
    transaction_df['transaction_id'] = pd.to_numeric(transaction_df['transaction_id'], errors='coerce')
    ...
    transaction_df['payment_method_id'] = pd.to_numeric(transaction_df['payment_method_id'], errors='coerce')
